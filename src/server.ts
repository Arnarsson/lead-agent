...
function longestCommonSubstr(a: string, b: string): number {
  // Returns ratio of LCS length to longer string length
  if (!a || !b) return 0;
  let max = 0;
...
function detectDuplicates(companies: { company: string; count: number; score: number; ids: string }[]): any[] {
  ...
  for (let i = 0; i < companies.length; i++) {
    ...
    if (group.leads.length > 1) {
      // Suggest keeping the one with the most leads / highest score
...
app.get('/api/run/log', (req, res) => {
  ...
  // Send buffered history to newly connected client
  ...
  try {
    const rows = db.prepare(`
      SELECT c.*,
        ...
        (SELECT target_name FROM targets t WHERE t.company_name = c.company_name LIMIT 1) as target_name,
        (SELECT outreach_strategy FROM targets t WHERE t.company_name = c.company_name LIMIT 1) as outreach_strategy
      FROM companies c
...
app.get('/api/duplicates', (_, res) => {
  const db = getDb();
  if (!db) return res.json([]);
  try {
    // Aggregate companies: name, total leads, avg score
    const rows = db.prepare(`
      SELECT company,
             COUNT(*) as count,
...
app.post('/api/leads/merge', (req, res) => {
  ...
  try {
    ...
    const deleted = db.prepare('SELECT changes() as c').get() as any;
